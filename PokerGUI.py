...
class PokerGUI:
    ...
    def _create_controls(self):
        ...
        self.strategy1.pack(pady=5)

        self.strategy2 =ttk.Combobox(threads_frame, values = strategies)
        self.strategy2.set("Strategy 1")
        self.strategy2.pack(pady=5)
    ...
    def _run_simulation(self):
            ...
            if strategy1_txt == strategy2_txt:
                messagebox.showerror("Invalid Input", "The strategies must be different")
                return
            ...
            simulation = PokerSimulationManager(strategy1_txt, strategy2_txt, num_games)
    ...
    def _update_results(self, results: Dict):
        ...
        if isinstance(results, dict) and 'player_stats' not in results:
        # Detectamos dict<strategy, stats_dict>
            strategies = list(results.keys())
            player_stats = []
            for strat in strategies:
                stats = results[strat] or {}
        ...
        # Actualizar dropdown de estrategias
        ...
        # Limpiar tablas
        for tree in (self.summary_tree, self.stats_tree, self.player_stats_tree):
        ...
        if 'Win Rate' in summary_stats.columns:
            for strat in summary_stats.index:
                wr = summary_stats.loc[strat, 'Win Rate']
                self.summary_tree.insert("", "end", values=(strat, f"{wr:.4f}"))
        else:
            for strat in strategies:
                mean_wr = df[df['Strategy']==strat]['Win Rate'].mean()
                self.summary_tree.insert("", "end", values=(strat, f"{mean_wr:.4f}"))
        ...
        self.ax1.set_facecolor('#202020'); self.ax2.set_facecolor('#202020')
        ...
        # Estadísticas por posición (solo si están presentes)
        for i, stats in enumerate(results['player_stats']):
            strat = results['strategies'][i] if i < len(results['strategies']) else 'Unknown'
            played = stats.get('hands_played', 0)
            won = stats.get('hands_won', 0)
            total_profit = stats.get('total_profit', 0)
            bl_succ = stats.get('bluffs_successful', 0)
            bl_att = stats.get('bluffs_attempted', 0)

            win_rate = won / max(1, played)
            bluff_rate = bl_succ / max(1, bl_att) if bl_att > 0 else 0

            pos_stats = stats.get('position_stats', {})
            pos_list = []
            for pos in ('early', 'middle', 'late'):
                p = pos_stats.get(pos, {})
                p_play = p.get('played', 0)
                p_won = p.get('won', 0)
                pos_list.append(f"{pos}: {p_won / max(1, p_play):.1%}")

                self.player_stats_tree.insert("", "end", values=(
                    strat,
                    played,
                    f"{win_rate:.1%}",
                    f"${total_profit:.2f}",
